...
* **Mercato P1-007 Final Head (`outbound/p1-007`):** `6e0ffab54622b79e1e21b76df6c9f2bb4bf32187`
* **Scanner P1-007 Final Head (`main`):** `b23325a7eb8c919d80d75a8a1eb3d58ef8a3d538`
...
* **R44 Individual Location Coverage & Unreserved Stock Boundary (2A5):** Replacement task is created only if an unblocked storage location individually possesses `unreservedGrossStock >= shortageQty`. If another active pick task already holds a hard reservation against candidate location B, the unreserved quantity is insufficient, halting auto-reallocation and escalating to Supervisor.
...
* **Outcome B (CANCEL_OR_CORRECT):**
  ...
  - **Zero Quantity (0 - Full Cancel):** Sets `CustomerOrderLine.status = 'CANCELLED'`, `OutboundOrderLine.status = 'CANCELLED'`, preserves positive baseline quantities to satisfy DB check constraints (`ordered_quantity > 0`, `required_qty > 0`), releases hard allocations and deletes linked inventory reservations. If `pickedQty > 0`, persists `wms_outbound_physical_return_handoffs` to ensure picked stock is returned.
...
## 3. Remote Testing PostgreSQL Integration Evidence (16/16 PASSED)
...
**Verbatim Output:**
```text
PASS src/modules/wms_outbound/services/__tests__/p1-007-postgres.integration.test.ts (55.432 s)
  P1-007 Genuine PostgreSQL SHORT_ALLOCATED & SHORT_PICKED Recovery Suite
    ...
    3. TC-062 SHORT_PICKED Supervisor Outcomes
      ...
      ✓ 3B: Outcome B (CANCEL_OR_CORRECT) rejects arbitrary positive values, accepts 0 (with physical return handoff) and exact picked quantity (updating CustomerOrderLine.orderedQuantity AND OutboundOrderLine.requiredQty per TC-121) (1510 ms)
...
## 4. Full Outbound Regression Gate (17/17 Suites, 241/241 PASSED)
...
```bash
yarn --cwd apps/mercato test src/modules/wms_inventory/services/__tests__/atp-service.test.ts
```
```text
Test Suites: 1 passed, 1 total
...
* **Execution Result:** `6 passed (41.2s)`
* **All 6 Distinct UI Outcomes Verified:**
  1. `Outcome 1: SHORT_ALLOCATED -> ALLOW_PARTIAL (persistent with reason)`
  2. `Outcome 2: SHORT_ALLOCATED -> CANCEL_OUTBOUND_ORDER`
  3. `Outcome 3: SHORT_PICKED -> WAIT`
  4. `Outcome 4: SHORT_PICKED -> CANCEL_OR_CORRECT (exact picked quantity 4)`
  5. `Outcome 5: SHORT_PICKED -> CANCEL_OR_CORRECT (0 cancellation)`
  6. `Outcome 6: SHORT_PICKED -> ALLOW_PARTIAL (persistent with mandatory reason)`
...
* **Execution Result:** `1 passed (9.1s)`
