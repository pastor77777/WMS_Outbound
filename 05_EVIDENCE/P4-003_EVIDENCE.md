...
Ran the exact targeted regression set P4-002's own accepted evidence used (P4-001, P3-003, P1-005, P1-006, P2-002, FND-003), plus P4-002 and P4-003 themselves, together in one `--forceExit` run:
...
- `apps/mercato`: `mercato db migrate` — `wms_outbound: 2 migrations applied` (this item's migration, plus one prior pending migration), all other modules `no pending migrations`.
...
- No unrelated work: no Return Receipt, Warehouse Transfer, PickWave, or carrier/ERP changes.
