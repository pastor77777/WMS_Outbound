...
- Mercato candidate commit SHA: `e30e3616013bfba3cbea13391119019e294fbc6e`
...
- Authority:
  - `proces_4_physical_putback.md` (P4 R1–R5, P4 Step 1: Decision on cancellation of picked / packed position)
  - `wymagania_outbound.md` (`FR-P5-14`, `FR-P5-15`, `FR-P5-16`)
...
1. **P3 vs P4 Discriminator**:
   ...
   - P4 handles **post-pick and post-pack** states:
     ...
     - Lines in `PICKED`
     - Lines in `PACKED`
2. **Supervisor Approval Requirement (P4 R1)**:
...
3. **Hard Cutoff at POSTING_PENDING (P1 R38, P4 R2)**:
...
4. **Immediate Logical Settlement (P4 R3)**:
   - Standard Allocation transitions to `RELEASED`.
...
5. **Durable Recovery Fact for P4-002**:
...
## Dedicated PostgreSQL Suite Mapping (16 Tests)

Suite: `apps/mercato/src/modules/wms_outbound/services/__tests__/p4-001-postgres.integration.test.ts`  
Result: **16/16 PASSED** (46.6s) on canonical Testing PostgreSQL (Supabase pooler).
...
- **P3-001 Reservation Release Suite**: `apps/mercato/src/modules/wms_outbound/services/__tests__/p3-001-reservation-release-postgres.integration.test.ts` — **13/13 PASSED** (77.7s).
- **P3-002 Reservation Retention Suite**: `apps/mercato/src/modules/wms_outbound/services/__tests__/p3-002-postgres.integration.test.ts` — **17/17 PASSED** (78.6s).
- **P1-001 CustomerOrder Lifecycle & Aggregation**: `apps/mercato/src/modules/wms_outbound/services/__tests__/p1-001-customer-order-lifecycle.test.ts` — **10/10 PASSED** (1.8s).
- **FND-001 Ordering Adapter & Routing**: `apps/mercato/src/modules/wms_outbound/services/__tests__/fnd-001-ordering-adapter.test.ts` — **10/10 PASSED** (1.8s).
...
- **Journey A (PACKED cancellation allowed with Supervisor approval)**:
  1. Seeds a genuine PACKED line into a Transport Unit attached to a Shipment in `DRAFT` (before `POSTING_PENDING`).
...
- **Journey B (PACKED cancellation blocked when Shipment is POSTING_PENDING)**:
  1. Seeds a genuine PACKED line attached to a Shipment in `POSTING_PENDING`.
  2. Navigates to `/backend/customer-orders/[id]`.
  ...
  6. Database verification confirms: order remains `IN_FULFILLMENT`, allocations `RESERVED`, zero physical return handoffs.
- **Journey C (PICKED line cancellation without supervisor approval requirement)**:
  1. Seeds a genuine PICKED line (pickedQty > 0, not packed).
  2. Navigates to `/backend/customer-orders/[id]`.
  3. Clicks `Check Cancellation (INT-06)`.
